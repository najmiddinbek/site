...
export default function EditTopicForm({ id, title, description, school, telNumber, darsQoldirish, manzili }) {
    ...
    const [newmanzili, setNewmanzili] = useState(manzili);
    ...
    function handleSubmit(e) {
        ...
        try {
            fetch(`http://localhost:3000/api/topics/${id}`, {
                ...
                body: JSON.stringify({ newTitle, newDescription, newSchool, newtelNumber, newdarsQoldirish, newmanzili }),
            })
                .then((res) => {
                    if (!res.ok) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            />
            <input
                onChange={(e) => setNewmanzili(e.target.value)}
                value={newmanzili}
                className="border border-slate-500 px-8 py-2"
                type="text"
